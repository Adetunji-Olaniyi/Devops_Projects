...
touch script.sh
![](./img/2.touchscriptsh.png)

Lets check the permission of the file

ls -latr script.sh

![](./img/3.permission.png)
...
For example, lets assume there is a user on the server called "john", a group on the server called "developers" and you want the owner of filename.txt changed from "Adetunji" to "john", and to also ensure that any user in the developer group has ownership of the file as well:
...
sudo adduser Adetunji (nb- you can use useradd)
...
The home directory represents a file system directory created in the name of the user. Such as /home/Teejay. This is where each user created on the server will store their respective data.

![](./img/11.adduseradetunji.png)

## Granting Administrative Privileges

By default, newly created user accounts do not have administrative privileges. To grant administrative access to a user, you can add the user to the sudo group. Users in the sudo group can run commands with administrative privileges. To the adetunji user to the sudo group, run:

sudo usermod -aG sudo 

![](./img/12.sudoadetunji.png)
...
Use theusermodcommand to add users to the group. For instance, to add users "akinwale and Lexzy to the 'developers' group:

sudo usermod -aG developers adetunji

![](./img/17.addtogroup.png)

The -aG options append the "developers" group to the users' existing group memberships.

## verifying Group Memberships
To confirm the group memberships for a specific user, use the id command. For example, to check the group memberships for the user "adetunji"

id adetunji

This command displays information about the user "adetunji" including the groups they belong to, such as "developers."

![](./img/20.checkstatus.png)
...
![](./img/18.devops%20group.png)

![](./img/19.createmultipleusers.png)

Add user to group

![](./img/21.adduserstogroup.png)

cat /etc/group

![](./img/22.catetcgroup.png)

create a folder for each user in the /home directory. For example /home/akinwale

sudo mkdir -p /home/users/Paul
sudo mkdir -p /home/users/Akinwale
sudo mkdir -p /home/users/Emmanuel
sudo mkdir -p /home/users/Ruth

![](./img/23.directoryforall.png)
